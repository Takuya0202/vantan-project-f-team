...
export async function GET() {
  ...
  const distancePerDay = navigations
    .filter((nav) => nav.createdAt >= today)
    .map((nav) => nav.distance)
  ...
  const distancePerWeek = navigations
    .filter((nav) => nav.createdAt >= week)
    .map((nav) => nav.distance)
    .reduce((prevDistance, currentDistance) => prevDistance + currentDistance, 0);

  // 合計の走行距離
  const totalDistance = navigations
    .map((nav) => nav.distance)
    .reduce((prevDistance, currentDistance) => prevDistance + currentDistance, 0);
